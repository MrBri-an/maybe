...
export default async function Home({ searchParams }: HomeProps) {
  ...
  if (access.ok) {
    if (await hasPassedExperienceGate(access.user.id)) {
      ...
      return <WorldExperience logoutAction={logout} initialView={getParam(params.view) === "world" ? "world" : "opening"} storybookCompleted={Boolean(journey?.progress.storybook_completed_at)} libraryCompleted={Boolean(journey?.progress.library_completed_at)} puzzleRoomCompleted={Boolean(journey?.progress.puzzle_room_completed_at)} radioCompleted={Boolean(journey?.progress.radio_completed_at)} questionGardenCompleted={Boolean(journey?.progress.question_garden_completed_at)} galleryCompleted={Boolean(journey?.progress.gallery_completed_at)} herUniverseCompleted={Boolean(journey?.progress.her_universe_completed_at)} maybeDaysCompleted={Boolean(journey?.progress.maybe_days_completed_at)} ourCornerCompleted={Boolean(journey?.progress.our_corner_completed_at)} finalWorldCompleted={Boolean(journey?.progress.final_world_completed_at)} initialDestination={journey?.progress.last_world_destination ?? null} />;
    }
