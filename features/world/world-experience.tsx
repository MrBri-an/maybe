...
type WorldExperienceProps = { logoutAction: () => Promise<void>; initialView?: "opening" | "world"; storybookCompleted?: boolean; libraryCompleted?: boolean; puzzleRoomCompleted?: boolean; radioCompleted?: boolean; questionGardenCompleted?: boolean; initialDestination?: string | null };
...
function Globe({ selected, onSelect, reduceMotion, storybookCompleted, libraryCompleted, puzzleRoomCompleted, radioCompleted, questionGardenCompleted }: { selected: number; onSelect: (index: number) => void; reduceMotion: boolean; storybookCompleted: boolean; libraryCompleted: boolean; puzzleRoomCompleted: boolean; radioCompleted: boolean; questionGardenCompleted: boolean }) {
...
function ProgressionGuide({ storybookCompleted, libraryCompleted, puzzleRoomCompleted, radioCompleted, questionGardenCompleted }: { storybookCompleted: boolean; libraryCompleted: boolean; puzzleRoomCompleted: boolean; radioCompleted: boolean; questionGardenCompleted: boolean }) {
...
export function WorldExperience({ logoutAction, initialView = "opening", storybookCompleted = false, libraryCompleted = false, puzzleRoomCompleted = false, radioCompleted = false, questionGardenCompleted = false, initialDestination = null }: WorldExperienceProps) {
  ...
  }, [libraryCompleted, puzzleRoomCompleted, radioCompleted, router, storybookCompleted, view]);
        ...
        {view === "opening" ? (
          ...
          <motion.main className="globe-view" key="globe" initial={{ opacity: 0, scale: reduceMotion ? 1 : .86 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: reduceMotion ? 0 : 1.1, ease: transition.ease }}>
            <header className="globe-header">
              ...
              <JourneyProgressMenu storybookCompleted={storybookCompleted} libraryCompleted={libraryCompleted} puzzleRoomCompleted={puzzleRoomCompleted} radioCompleted={radioCompleted} questionGardenCompleted={questionGardenCompleted} />
            </header>
            <Globe selected={selectedRoom} onSelect={selectRoom} reduceMotion={reduceMotion} storybookCompleted={storybookCompleted} libraryCompleted={libraryCompleted} puzzleRoomCompleted={puzzleRoomCompleted} radioCompleted={radioCompleted} questionGardenCompleted={questionGardenCompleted} />
            <aside className="globe-detail" aria-live="polite"><div><Badge tone={selectedRoom <= 4 && (selectedRoom !== 1 || storybookCompleted) && (selectedRoom !== 2 || libraryCompleted) && (selectedRoom !== 3 || puzzleRoomCompleted) && (selectedRoom !== 4 || radioCompleted) ? "gold" : "neutral"}>{selectedRoom === 0 ? storybookCompleted ? "Completed" : "Available" : selectedRoom === 1 ? libraryCompleted ? "Completed" : storybookCompleted ? "Available" : "Locked" : selectedRoom === 2 ? puzzleRoomCompleted ? "Completed" : libraryCompleted ? "Available" : "Locked" : selectedRoom === 3 ? radioCompleted ? "Completed" : puzzleRoomCompleted ? "Available" : "Coming later" : selectedRoom === 4 && questionGardenCompleted ? "Completed" : selectedRoom === 4 && radioCompleted ? "Available" : "Coming later"}</Badge><h2>{selected.name}</h2><p>{selectedRoom === 0 ? storybookCompleted ? "The first story is complete. You can return whenever you like." : "Begin with the story of the screenshot that started everything." : selectedRoom === 1 ? libraryCompleted ? "This quiet reading room is open whenever you want to return." : storybookCompleted ? "A private reading room for books, stories and discoveries." : "Complete the Storybook to open this quiet reading room." : selectedRoom === 2 ? puzzleRoomCompleted ? "The clues remain here whenever you feel like returning." : libraryCompleted ? "A playful room of clues and small discoveries." : "Complete the Library journey to open this playful room." : selectedRoom === 3 ? radioCompleted ? "The music library remains open whenever you want to return." : puzzleRoomCompleted ? "A private shared music library for favourite songs and discoveries." : "This destination will open later as the world continues to grow." : selectedRoom === 4 && radioCompleted ? "A private garden of thoughtful questions, always optional." : "This destination will open later as the world continues to grow."}</p>{selectedRoom === 0 ? <Link className="button button-primary button-small globe-enter-story" href="/story" prefetch>{storybookCompleted ? "Open Storybook" : "Enter Storybook"}</Link> : null}{selectedRoom === 1 && storybookCompleted ? <Link className="button button-primary button-small globe-enter-story" href="/library" prefetch>{libraryCompleted ? "Visit Library" : "Enter Library"}</Link> : null}{selectedRoom === 2 && libraryCompleted ? <Link className="button button-primary button-small globe-enter-story" href="/puzzles" prefetch>{puzzleRoomCompleted ? "Visit Puzzle Room" : "Enter Puzzle Room"}</Link> : null}{selectedRoom === 3 && puzzleRoomCompleted ? <Link className="button button-primary button-small globe-enter-story" href="/radio" prefetch>{radioCompleted ? "Visit Jessica’s Radio" : "Enter Jessica’s Radio"}</Link> : null}{selectedRoom === 4 && radioCompleted ? <Link className="button button-primary button-small globe-enter-story" href="/question-garden" prefetch>{questionGardenCompleted ? "Visit Question Garden" : "Enter Question Garden"}</Link> : null}</div><span aria-hidden="true">{String(selectedRoom + 1).padStart(2, "0")}</span></aside>
            <ProgressionGuide storybookCompleted={storybookCompleted} libraryCompleted={libraryCompleted} puzzleRoomCompleted={puzzleRoomCompleted} radioCompleted={radioCompleted} questionGardenCompleted={questionGardenCompleted} />
          </motion.main>
        )}
      </AnimatePresence>
    </div>
