...
export async function authorizeMaybeDays() {
  ...
  if (!progress?.storybook_completed_at
    ...
    || !progress.her_universe_completed_at) return null;
  const admin = createAdminSupabaseClient();
  return admin ? { access, admin, progress } : null;
}

export async function recordMaybeDaysLocation(
  authorized: NonNullable<Awaited<ReturnType<typeof authorizeMaybeDays>>>,
) {
  const { error } = await authorized.admin.from("user_journey_progress")
    .update({ last_location: "maybe-days", last_world_destination: "maybe-days" })
    .eq("user_id", authorized.access.user.id);
  if (error) console.error("Maybe Days foundation operation failed", { operation: "record_location", code: error.code });
}
