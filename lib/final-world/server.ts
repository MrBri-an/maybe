...
export async function authorizeFinalWorld() {
  ...
  if (!progress?.storybook_completed_at
    ...
    || !progress.maybe_days_completed_at
    || !progress.our_corner_completed_at) return null;
  const admin = createAdminSupabaseClient();
  return admin ? { access, admin, progress } : null;
}

export async function recordFinalWorldLocation(
  authorized: NonNullable<Awaited<ReturnType<typeof authorizeFinalWorld>>>,
) {
  const { error } = await authorized.admin.from("user_journey_progress")
    .update({
      last_location: "the-world-i-can-give-you",
      last_world_destination: "the-world-i-can-give-you",
    })
    .eq("user_id", authorized.access.user.id);
  if (error) console.error("Final world foundation operation failed", { operation: "record_location", code: error.code });
}
